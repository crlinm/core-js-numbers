...
function roundToPowerOfTen(num, pow) {
  const tenPow = Number('1'.concat('0'.repeat(pow)));
...
function isNumber(number) {
  return Number.isFinite(number) && typeof number === 'number';
}
...
function getCountOfOddNumbers(number) {
  ...
  return Array.from({ length: end - start + 1 }, (v, k) => k + start).filter(
    (item) => Math.abs(item % 2) === 1
  ).length;
}
